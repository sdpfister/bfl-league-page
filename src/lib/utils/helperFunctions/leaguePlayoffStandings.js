...
import {playoffStandingsStore} from '$lib/stores';
import { round } from './universalFunctions';

export const getLeaguePlayoffStandings = async () => {
	if(get(playoffStandingsStore).standingsInfo) {
		return get(playoffStandingsStore);
	}
	...
	// Sort by wins first, then by fpts (points for)
    const sortedStandings = Object.values(standings).sort((a, b) => b.wins - a.wins || b.fpts - a.fpts);

    // Take the top 5 teams based on wins
    const topFiveTeams = sortedStandings.slice(0, 5);
    ...
    // Combine them to get the top 6 teams for playoffs
    const playoffStandings = [...topFiveTeams, sixthTeam];

	const response = {
		standingsInfo: playoffStandings,
		yearData,
		medianMatch
	}
	
	playoffStandingsStore.update(() => response);
